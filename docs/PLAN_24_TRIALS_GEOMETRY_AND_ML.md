...
**Goal:** Use the 24 trials to choose better **reference statistics** and **pipeline parameters** so that when you run `marker-label` on a new subject (e.g. BBA01), the geometry-based matching is more reliable.

### 1.1 Inputs

- **Per trial (×24):**
  - One **static** C3D for that subject (BBpilot01 or BBpilot02), already labeled.
  - One **dynamic** C3D for that subject, **manually labeled** (same marker names as static).
- **Units:** If any dynamic is in meters, use `--dynamic-unit m` when loading that file (as you did for BBpilot01 Trial 10).
...
**Caveat (subject- and stride-dependence):** The “best frame” fraction (where in the trial the frame closest to the template falls) can be **subject-specific** and **stride-dependent** (보폭, walking speed, trial type). So an aggregated window from BBpilot01/02 may not transfer well to BBA01. Use the aggregated window as a **loose prior** (e.g. search in 0.2–0.8 but optionally bias toward the reference fraction), or prefer a **wider** middle range and rely more on **Procrustes at best frame** or a **per-trial** “closest to template” criterion (see below) when labeling a different subject.
...
**Step 1d – Use when labeling BBA01**

When you run the pipeline for the **new** subject (BBA01):

```bash
marker-label "BBA01 Cal 01.c3d" "BBA01 Trial 05.c3d" -o out/BBA01_trial05 \
...
- **Improved geometry-based labels** on BBA01 (or any new subject) when you run `marker-label` with these parameters.
...
**Goal:** Train a model that takes a **single frame’s 3D point cloud** (and optionally context) and predicts **which label each point has**. Use the 24 trials as training (and validation) data, then apply the model to **new trials** (e.g. BBA01) or to held-out trials for evaluation.
...
- **Consistent label set:** All 24 trials (and, for application, BBA01) should use the **same marker names** (e.g. same 43 labels). If BBpilot01/02 have 141 markers and BBA01 has 43, you must either:
  - Restrict to the **common 43 labels** for training and application, or
  - Define a mapping (e.g. “body part” or “reduced set”) so that the model’s output can be interpreted on BBA01.
...
- **Split options:**
  ...
  - **Leave-one-subject-out:** Train on all trials from BBpilot01, validate on all from BBpilot02 (or vice versa). Repeat for the other subject. This measures **cross-subject** generalization.
...
- **Validation:** Run on the chosen validation split (by trial or by subject). If you do leave-one-subject-out, you get two numbers: performance when generalizing from BBpilot01→BBpilot02 and from BBpilot02→BBpilot01. That tells you how well the model transfers between these two subjects.

### 2.5 Application to BBA01 (or a new trial)

- **At inference:** For each frame of BBA01’s unlabeled dynamic trial, you have `(N, 3)` points (N may be 43 or whatever the trial has). Run the model → get predicted label per point. No need for static or template; the model directly maps point cloud → labels.
...
- **Predicted labels** for BBA01 (or any new trial) when you run the model on that trial’s frames; optionally **smoothed or hybrid** with propagation.
...
- **Does not:** Replace the need for consistent labels and units; if BBA01 has a different marker set, you still need a common subset or mapping. It also does not use the static trial at inference (unless you add it as an optional input to the model).
...
| **Output** | Aggregated reference report or fixed CLI parameters; better geometry-based labels on BBA01 | Trained model; predicted labels on BBA01 or held-out trials |
| **Evaluation** | Compare labeling quality on BBA01 (or leave-one-out on 24) when using new params vs old | Validation accuracy; leave-one-subject-out or leave-one-trial-out |
